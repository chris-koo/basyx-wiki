...
To search by serial number, encode the complete JSON value `{"name":"serialNumber","value":"SN-001"}` using the encoding commands above, then replace `ENCODED_SPECIFIC_ASSET_ID`:
...
Collection responses return a `result` array and `paging_metadata`. If `paging_metadata.cursor` is present and non-empty, request the next page with that cursor and the same filters:

```bash
curl -i -G http://localhost:8082/shell-descriptors --data-urlencode 'updatedFrom=2026-09-02T10:00:00Z' --data-urlencode 'limit=10' --data-urlencode 'cursor=RETURNED_CURSOR'
```

Replace `RETURNED_CURSOR` with the actual returned value. Stop when no next cursor is supplied. A database containing only the example descriptor may fit on one page.
...
Expect `200 OK` with a paged descriptor result. String values in the query JSON are unencoded. For subsequent pages, submit the same query with the returned cursor as a URL-escaped query parameter. See the [query language examples](https://github.com/eclipse-basyx/basyx-go-components/blob/main/docu/query_language/examples.md) for combinations and nested descriptor filters, and the running Swagger UI for the installed version's contract.
